...
let driver = new Builder().forBrowser('chrome').setChromeOptions(
  new chrome.Options().addArguments(['--headless', '--no-sandbox', '--kiosk', 'window-size=1920,1080'])).build();
...
module.exports = {
  ...
  copyTerminalTextToClipboard: async function() {
    ...
    await driver.actions({bridge: true}).move({x: parseInt(textLayerSize.x), y: parseInt(textLayerSize.y)})
